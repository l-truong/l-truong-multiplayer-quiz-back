...
describe('POST /questions', () => {
    it('should return 400 error if missing parameters', async () => {
        ...
        const resOnlyImageUrl = await request(app).post('/questions').send(newOnlyImageUrl);
    ...
    it('should return 400 error if rest of parameters not string', async () => {
        ...
        let resInvalidImageUrl = await request(app).post('/questions').send(newInvalidImageUrl);
        expect(resInvalidImageUrl.status).toBe(400);
        expect(resInvalidImageUrl.body.message).toBe('An error occurred');
        expect(resInvalidImageUrl.body.error).toBe('Parameters must be strings');
...
describe('POST /questions/bulk', () => {
    ...
    it('should return 400 error if parameters are not strings', async () => {
        ...
        expect(res.body.errors[5].error).toBe('Parameters must be strings');
        expect(res.body.errors[5].question).toMatchObject(newQuestions[5]);
...
describe('PATCH /questions/:id', () => {
    ...
    it('should return 400 error if parameters are not a string', async () => {
        ...
        const resImageUrlNotString = await request(app).patch(`/questions/${mockValue._id.toString()}`).send({ imageUrl: 4 });
        expect(resImageUrlNotString.status).toBe(400);
        expect(resImageUrlNotString.body.message).toBe('An error occurred');
        expect(resImageUrlNotString.body.error).toBe('Parameters must be strings');
        expect(resImageUrlNotString.body.invalidParams).toEqual({ imageUrl: 4 });

        const resAllParametersNotString = await request(app).patch(`/questions/${mockValue._id.toString()}`).send({ questionText: 0, correctAnswer: 1, explanation: 2, categoryId: 3, imageUrl: 4 });
    ...
    it('should update a question', async () => {
        ...
        const resImageUrl = await request(app).patch(`/questions/${mockValue._id.toString()}`).send({ imageUrl: 'http://imageUrl10' });
